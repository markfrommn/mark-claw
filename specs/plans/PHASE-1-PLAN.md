...
3. **`mclaw doctor --init` exits 0 on successful scaffolding**, regardless of hard FAILs on deferred downstream config (an unset vault path). `--init` is a bootstrap command; its exit code reflects tree-creation success, not whether C1-owned config is filled in. Bare `mclaw doctor` (the validator) keeps nonzero-on-hard-FAIL semantics, so an unset vault still surfaces there. Consequence: the "all green" AC holds once the operator sets the vault path (C1/DEV-18 creates the dir; until then an absolute-but-absent vault is WARN, not FAIL).
