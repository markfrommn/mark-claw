...
`mclaw_core/output_guard.py` compiles a deny-pattern set from `exclusions.yaml` — every blocked/ephemeral entry's IDs, names, aliases, and per-entry `also_match:` strings, matched case-insensitively with two strategies: **token identifiers** (IDs, names, aliases, `also_match`) anchor on non-word boundaries (`(?<!\w)…(?!\w)`) so an identifier containing punctuation still matches without substring false-positives; **substring identifiers** (drive paths **and** meeting titles) match as literal substrings, since both are free-text-span identifiers the fetch gate matches by containment — a path appears in free text as a contiguous span (e.g. `/HR/payroll`) where `\b` would reject the match (the char after `/HR/` is a word char), and a title `Comp review` must trip `Comp reviews Q3` exactly as the gate blocks it (a trailing `s` would defeat `(?!\\w)`). Substring is broader and therefore fail-closed (more quarantine, no leak). Ephemeral entries contribute their identifiers for *persistence* surfaces only, since alerts legitimately name them as pointers. Every writer calls it on the full artifact before emitting:
...
- **On hit — fail closed:** the artifact is **not** written/sent. It is moved to `state/quarantine/` (0700, outside the vault and outside any sync), a `guard.trip` changelog record is written, a needs-sign-off review item is queued with the match context, and a system-severity Telegram alert is dispatched ("exclusion guard tripped on morning-briefing — artifact quarantined"). The Telegram alert is Phase 2 — the notify layer does not exist yet, so the guard's trip-alert hook is currently a no-op stub; until it is wired, an operator must NOT assume a trip was alerted. The quarantine + changelog + review-queue path is fully functional without the alert. A tripped briefing is *lost until reviewed* — accepted cost; false-positive patterns get refined via the review loop (`also_match`/alias tuning is config, user-owned).
