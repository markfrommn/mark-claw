...
`mclaw_core/output_guard.py` compiles a deny-pattern set from `exclusions.yaml` — every blocked/ephemeral entry's IDs, names, aliases, and per-entry `also_match:` strings, matched case-insensitively with two strategies: **token identifiers** (IDs, names, aliases, `also_match`, meeting titles) anchor on non-word boundaries (`(?<!\w)…(?!\w)`) so an identifier containing punctuation still matches without substring false-positives; **drive-path identifiers** match as literal substrings, since a path appears in free text as a contiguous span (e.g. `/HR/payroll`) where `\b` would reject the match (the char after `/HR/` is a word char) — substring is broader and therefore fail-closed (more quarantine, no leak). Ephemeral entries contribute their identifiers for *persistence* surfaces only, since alerts legitimately name them as pointers. Every writer calls it on the full artifact before emitting:
